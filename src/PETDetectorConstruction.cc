...
void PETDetectorConstruction::BuildMaterial() {
  ...
  //Silica Gel
  fGel = new G4Material("Gel", 2.21 * g/cm3, 1);
  fGel->AddMaterial(fSiliconDioxide, 100* perCent);
  ...
  //Rise and Decay Time  https://iopscience.iop.org/article/10.1088/1748-0221/7/09/P09004/pdf
  ...
  fLYSO_MPT->AddConstProperty("RISETIMECONSTANT", 72 * ps);

  fAir_MPT->AddProperty("RINDEX", ene, rAir, num);

  fGel_MPT->AddConstProperty("RINDEX",  1.82);
...
G4VPhysicalVolume * PETDetectorConstruction::Construct() {
  ...
  G4double cryst_dX = 24.7*mm, cryst_dY = 24.7*mm, cryst_dZ = 20*mm, det_dZ = 1*mm;
  G4int nb_cryst = 13;
  ...
  std::cout<<ring_R1<<std::endl;


  G4double gap = 0.2*mm;
  G4double dX = cryst_dX - gap, dY = cryst_dY - gap;

  //solidCryst = new G4Box("crystal", dX/2, dY/2, cryst_dZ/2);
  G4VSolid* solidCryst = new G4Box("crystal", dX/2, dY/2, cryst_dZ/2);
  //  logicCryst = new G4LogicalVolume(solidCryst, fLYSO, "CrystalLV");

  solidDet_outter = new G4Box("detector", dX/2, dY/2, det_dZ/2);
  solidDet_inner = new G4Box("detector", dX/2, dY/2, det_dZ/2);
  logicDet_outter = new G4LogicalVolume(solidDet_outter, fAir, "DetectorLV");
  logicDet_inner = new G4LogicalVolume(solidDet_outter, fAir, "DetectorLV");



  G4double pDx1= dX/2, pDx2=dX/2, pDz= (cryst_dZ-gap)/2, pTheta =0, pPhi =0, pDy1 = gap/2, pDy2 = (10.5*mm)/2, pDx3 = (cryst_dZ+gap)/2, pDx4 = (cryst_dZ+gap)/2, pAlp1 = 0*degree,pAlp2 = 0*degree ;

  solidGel = new G4Trap("gel", pDz, pTheta, pPhi, pDy1, pDx1, pDx2, pAlp1, pDy2, pDx3, pDx4, pAlp2);
  //solidGel = new G4Tubs("gel", 0.5*ring_R1, 0.5*ring_R2, cryst_dZ/2, 0, twopi/32);
  //solidGel = new G4Trap("gel", cryst_dZ+2*det_dZ, dY, 2*cryst_dZ+gap, gap);
  G4VSolid* Y_gelBox = new G4Box("gelbox", (dX/2), 0.1*mm, cryst_dZ/2);
    G4VSolid* X_gelBox = new G4Box("gelboxx", 0.1*mm, (dY/2), cryst_dZ/2);

  logicGel = new G4LogicalVolume(solidGel, fGel, "GelLV");
   //-2*gap/ring_R2, 2*gap/ring_R2);
  ...
  G4ThreeVector T1(0, -9.3*mm, 0);
  G4ThreeVector T2(0, -6.2*mm, 0);
  G4ThreeVector T3(0, -3.1*mm, 0);
  G4ThreeVector T4(0, 0, 0);
  G4ThreeVector T5(0, 3.1*mm, 0);
  G4ThreeVector T6(0, 6.2*mm, 0);
  G4ThreeVector T7(0, 9.3*mm, 0);
  G4ThreeVector T8(-9.3*mm, 0, 0);
  G4ThreeVector T9(-6.2*mm, 0, 0);
  G4ThreeVector T10(-3.1*mm, 0, 0);
  G4ThreeVector T11(0, 0, 0);
  G4ThreeVector T12(3.1*mm, 0, 0);
  G4ThreeVector T13(6.2*mm, 0, 0);
  G4ThreeVector T14(9.3*mm, 0, 0);
  G4VSolid* sub1 = new G4SubtractionSolid("asdf", solidCryst, Y_gelBox, 0, T1);
  G4VSolid* sub2 = new G4SubtractionSolid("gg", sub1, Y_gelBox, 0, T2);
  G4VSolid* sub3 = new G4SubtractionSolid("gg", sub2, Y_gelBox, 0, T3);
  G4VSolid* sub4 = new G4SubtractionSolid("gg", sub3, Y_gelBox, 0, T4);
  G4VSolid* sub5 = new G4SubtractionSolid("gg", sub4, Y_gelBox, 0, T5);
  G4VSolid* sub6 = new G4SubtractionSolid("gg", sub5, Y_gelBox, 0, T6);
  G4VSolid* sub7 = new G4SubtractionSolid("gg", sub6, Y_gelBox, 0, T7);
  G4VSolid* sub8 = new G4SubtractionSolid("gg", sub7, X_gelBox, 0, T8);
  G4VSolid* sub9 = new G4SubtractionSolid("gg", sub8, X_gelBox, 0, T9);
  G4VSolid* sub10 = new G4SubtractionSolid("gg", sub9, X_gelBox, 0, T10);
  G4VSolid* sub11 = new G4SubtractionSolid("gg", sub10, X_gelBox, 0, T11);
  G4VSolid* sub12 = new G4SubtractionSolid("gg", sub11, X_gelBox, 0, T12);
  G4VSolid* sub13 = new G4SubtractionSolid("gg", sub12, X_gelBox, 0, T13);
  G4VSolid* sub14 = new G4SubtractionSolid("gg", sub13, X_gelBox, 0, T14);

  G4VSolid* uni1 = new G4UnionSolid("ezbby", Y_gelBox, X_gelBox, 0, T11);
  G4VSolid* uni2 = new G4UnionSolid("ezbby", uni1, X_gelBox, 0, T12);
  G4VSolid* uni3 = new G4UnionSolid("ezbby", uni2, X_gelBox, 0, T13);
  G4VSolid* uni4 = new G4UnionSolid("ezbby", uni3, X_gelBox, 0, T14);
  G4VSolid* uni5 = new G4UnionSolid("ezbby", uni4, X_gelBox, 0, T10);
  G4VSolid* uni6 = new G4UnionSolid("ezbby", uni5, X_gelBox, 0, T9);
  G4VSolid* uni7 = new G4UnionSolid("ezbby", uni6, X_gelBox, 0, T8);
  G4VSolid* uni8 = new G4UnionSolid("ezbby", uni7, Y_gelBox, 0, T1);
  G4VSolid* uni9 = new G4UnionSolid("ezbby", uni8, Y_gelBox, 0, T2);
  G4VSolid* uni10 = new G4UnionSolid("ezbby", uni9, Y_gelBox, 0, T3);
  G4VSolid* uni11 = new G4UnionSolid("ezbby", uni10, Y_gelBox, 0, T5);
  G4VSolid* uni12 = new G4UnionSolid("ezbby", uni11, Y_gelBox, 0, T6);
  G4VSolid* uni13 = new G4UnionSolid("ezbby", uni12, Y_gelBox, 0, T7);

  logicFillerGel = new G4LogicalVolume(uni13, fGel, "Filler_GelLV");
  logicCryst = new G4LogicalVolume(sub14, fLYSO, "CrystalLV");
  //logicCryst = new G4LogicalVolume(solidCryst, fLYSO, "CrystalLV");

  for (G4int icrys = 0; icrys < nb_cryst ; icrys++) {
    G4double phi = icrys*dPhi;
    G4double gphi = icrys*dPhi+half_dPhi;
    G4RotationMatrix rotm = G4RotationMatrix();
    rotm.rotateY(90*degree);
    rotm.rotateZ(phi);

    G4RotationMatrix rotd = G4RotationMatrix();
    rotd.rotateY(90*degree);
    rotd.rotateZ(phi);
    ...
    G4ThreeVector ug = G4ThreeVector(std::cos(gphi),  std::sin(gphi), 0);
    G4ThreeVector position_cryst = (ring_R1+0.5*cryst_dZ)*uz;
    G4ThreeVector position_det_outter = (ring_R1+cryst_dZ+det_dZ/2)*uz;
    G4ThreeVector position_det_inner = (ring_R1-0.5*det_dZ)*uz;
    G4ThreeVector position_gel = (ring_R1+12.5*mm)*ug;
    ...
    new G4PVPlacement(transform_det_outter,             //rotation,position
    		      ...
    		      icrys,                 //copy number
    		      checkOverlaps);       // checking overlaps
    new G4PVPlacement(transform_det_inner,             //rotation,position
    ...
    new G4PVPlacement(transform_gel,
    		      ...
    		      checkOverlaps);
    new G4PVPlacement(transform_cryst,             //rotation,position
              logicFillerGel,            //its logical volume
              "FillerGel",             //its name
              worldLog,//logicRing,             //its mother  volume
              false,                 //no boolean operation
              icrys,                 //copy number
  ...
  G4VisAttributes* GelVisAttributes = new G4VisAttributes(G4Colour(1.0,0.0,0.0));

  logicGel->SetVisAttributes(GelVisAttributes);
  logicFillerGel->SetVisAttributes(GelVisAttributes);

  logicDet_outter->SetVisAttributes(SiPMVisAttributes);
  logicDet_inner->SetVisAttributes(SiPMVisAttributes);
